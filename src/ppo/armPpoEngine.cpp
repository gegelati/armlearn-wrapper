...
#include "armPpoEngine.h"


double ArmPPOEngine::runOneEpisode(uint16_t seed, Learn::LearningMode mode, uint16_t iterationNumber){
    ...
    std::pair<torch::Tensor, torch::Tensor> actionNCritic;
    ...
    while (!terminated && nbActions < maxNbActions) {
        ...
        if(!ppoParams.multipleActions){
            ...
            if(ppoParams.falseSingleAction){
            ...
            if (!ppoParams.falseSingleAction || nbNullAction == actionVector.size()){
        ...
        if(mode == Learn::LearningMode::TRAINING){
            ...
            // Remember data
            learningAgent.remember(state, actionNCritic, singleReward, terminated);

            // Learn
            if(learningAgent.getBufferSize() == ppoParams.sizeBuffer){
                learningAgent.learn();
            }
...
std::vector<float> ArmPPOEngine::doOneActionInference(){
...
void ArmPPOEngine::trainOneGeneration(uint16_t nbIterationTraining){
...
void ArmPPOEngine::validateOneGeneration(uint16_t nbIterationValidation){
...
void ArmPPOEngine::testingModel(uint16_t nbIterationTesting){
    double result = 0;
    double success = 0;
    double distance = 0;
...
void ArmPPOEngine::chronoFromNow(){
    checkpoint = std::make_shared<std::chrono::time_point<
    std::chrono::system_clock, std::chrono::nanoseconds>>(std::chrono::system_clock::now());
}

void ArmPPOEngine::logNewGeneration()
{
...
void ArmPPOEngine::logTraining(double distance, double result){
...
void ArmPPOEngine::logTrainingValidation(double distance){
...
double ArmPPOEngine::getLastTrainingValidationScore(){
    return lastTrainingValidationScore;
}

double ArmPPOEngine::getLastTrainingScore(){
    return lastTrainingScore;
}

torch::Tensor ArmPPOEngine::getTensorState(){
    // Create zero tensor
